...
def help():
    print_help(r"""
: set USERNAME              The target username you want to find
...
def console():
    while True:
        try:
            user_in = input("CyberCrack: CyberServe/osint/username > ")
            if user_in.startswith("set") and user_in.split()[1] == "USERNAME":
                user = user_in.split()[2]
                print("USERNAME = " + user)
            elif user_in.lower() == "help" or user_in.lower() == "show options":
                help()
